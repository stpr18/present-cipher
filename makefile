PROGRAM_NAME := present_cipher

ifeq ($(OS),Windows_NT)
	CXX := D:\system\TDM-GCC-64\bin\g++
	MKDIR = if NOT EXIST $(subst /,\,$(1)) md $(subst /,\,$(1))
	BOOST_PATH := D:\system\boost\include\boost
	BOOST_LIB_PATH := D:\system\boost\lib\x64\lib
else
	CXX := g++
	MKDIR = mkdir -p $(1)
	BOOST_PATH := $(HOME)/local/boost/include
	BOOST_LIB_PATH := $(HOME)/local/boost/lib
endif

CXXFLAGS := -Wall -Wextra -lm -O3 -march=native -fomit-frame-pointer -fopenmp -std=c++11 -Wno-missing-field-initializers -I$(BOOST_PATH) -L$(BOOST_LIB_PATH)
LIBS := 
